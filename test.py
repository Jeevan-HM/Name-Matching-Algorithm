...
try:
    ...
    class test_levenshtein_ratio(unittest.TestCase):

        # ? Check if levenshtein ratio works!
        def test_levenshtein_ratio_success(self):
            tester = app.test_client(self)
            response = tester.get("/name/" + pass_test_name_1 + "/" + pass_test_name_2)
            data = json.loads(response.get_data(as_text=True))
            levenshtein_ratio = data["levenshtein_ratio"]
            assertion_levenshtein_ratio = 0
            if levenshtein_ratio >= 85:
                assertion_levenshtein_ratio = 1
            print(
                "[Success]Names match, levenshtein ratio [%d] between '%s' and '%s'"
                % (levenshtein_ratio, pass_test_name_1, pass_test_name_2)
            )
            self.assertEqual(assertion_levenshtein_ratio, 1)

        # ? Check if levenshtein distance fails test case
        def test_levenshtein_ratio_failure(self):
            tester = app.test_client(self)
            response = tester.get("/name/" + fail_test_name_1 + "/" + fail_test_name_2)
            data = json.loads(response.get_data(as_text=True))
            levenshtein_ratio = data["levenshtein_ratio"]
            ...
            if levenshtein_ratio < 85:
                assertion_levenshtein_ratio = 0
            print(
                "[Success]Names don't match, levenshtein ratio [%d] between '%s' and '%s'"
                % (levenshtein_ratio, fail_test_name_1, fail_test_name_2)
            )

            self.assertEqual(assertion_levenshtein_ratio, 0)

        # ? Check if the levenshtein params are empty
        def test_levenshtein_ratio_failure_no_params(self):
            tester = app.test_client(self)
            response = tester.get("/name/" + empty_input_1 + "/" + empty_input_2)
            assertion_levenshtein_ratio = 1
            if empty_input_1 == "" or empty_input_2 == "":
                assertion_levenshtein_ratio = 0
            print("[Success]Both Params are empty for levenshtein ratio")
            self.assertEqual(assertion_levenshtein_ratio, 0)

        # ? Check if the levenshtein first param is empty
        def test_levenshtein_ratio_failure_empty_first_param(self):
            tester = app.test_client(self)
            response = tester.get("/name/" + empty_input_1 + "/" + pass_test_name_1)
            assertion_levenshtein_ratio = 1
            if empty_input_1 == "" or pass_test_name_1 == "":
                assertion_levenshtein_ratio = 0
            print("[Success]First Param is empty for levenshtein ratio")
            self.assertEqual(assertion_levenshtein_ratio, 0)
